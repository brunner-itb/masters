...
class FEMMesh:
	...
	def __init__(self, Mesh, Name, Gender, Source, h_real, u_max_start, SaveFile, Dimension = 3): #createMesh = True):
		#if createMesh == False:
		...
		if activeSurfaceSource == True:
			...
			self.stimulusCellIndex = self.stimulusCell.index()
		...
		self.cell_to_edges_map = cell_to_edges_map(self)
		self.edges_to_cells_map = edges_to_cells_map(self)
		...
		self.vertex_acceleration_list = [None]*self.boundaryMesh.num_vertices()

		self.vertex_velocity = np.asarray([[0,0,0]]*self.boundaryMesh.num_vertices(), float)

		self.distance_to_grow = [None]*self.boundaryMesh.num_vertices()
		self.distance_to_grow_history = [0]*num_steps
		

		self.cell_volumes = cell_volumes(self)
		self.initial_cell_volumes = self.cell_volumes
		self.minimumCellVolume = None

		self.solutionFunctionRange = 0.4
		self.initialRange = 0.15
		self.number_of_refinements = 0

		self.maximumCurrentSolutionFunctionValue = None
		self.maxValue_corresponding_cell = None
		self.minimumCurrentSolutionFunctionValue = None
		self.minValue_corresponding_cell = None
